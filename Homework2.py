...
if __name__ == '__main__':
    ...
    parser.add_argument('size', type=int, nargs='?', default=60, metavar='PACKET_SIZE', help='IP packet size, include header size')
    ...
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_RAW, send_protocol) as send_socket:
            ...
            for hop in range(1, max_hop + 1):
                ...
                for request_packet in generate_request(sock=send_socket, count=3, echo_id=echo_id, length=packet_size, ttl=hop, protocol=send_protocol, udp_port=port):
                    ...
                    with socket.socket(socket.AF_INET, socket.SOCK_RAW, socket.IPPROTO_ICMP) as recv_socket:
                        ...
                        received_time, addr, finished = recv_resp(sock=recv_socket, echo_id=echo_id, protocol=send_protocol, timeout=timeout, sent_packet=request_packet)
